function thumbnailTemplateHTML(i) {
  ...
  <div onclick="loadPokemonInfos(${i})" class="pokemon_card thumbnail_bgr_${loadedPokemonData[i].types[0].type.name}" id="pokemon_id_${loadedPokemonData[i].id}">
  <div class="pokemon_card_info">
    <h4>#${createIdDigit(i)}</h4>
    <h3>${upperCasePokemonName(i)}</h3>
    <div class="pokemon_card_types" id="pokemon_card_types_${loadedPokemonData[i].id}">
    </div>
  </div>
...
function pokemonEvolutionChainTemplateHTML(i) {
  document.getElementById(`infos_${loadedPokemonData[i].name}`).innerHTML = '';
  document.getElementById(`infos_${loadedPokemonData[i].name}`).innerHTML += `
    <div><h2>Evolution</h2></div>
    <div id="evolution_chain_container" class="evolution_chain_row">
      <div class="pokemon_evolutin_png_container">
        <img class="pokemon_png_evolution_size" src="${loadedPokemonData[i].sprites.front_default}">
        <div>Name: ${loadedPokemonEvolutionChain[i].chain.species.name}</div>
      </div>
      <div class="pokemon_evolution_infos">
      ...
      <div class="pokemon_evolutin_png_container">
        ...
        <div>Name: ${loadedPokemonEvolutionChain[i].chain.evolves_to[0].species.name}</div>
      </div>
      <div class="pokemon_evolution_infos">
      <div class="evolution_arrow_container"><div>&raquo;</div></div>
    </div>
